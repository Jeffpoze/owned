...
''', today: today);
    ...
    expect(r.lines.map((l) => l.description), [
      'SAMSUNG 65" QN90A TV',
      '4 YR PROTECTION PLAN',
    ]);
...
''', today: today);
  ...
  test(
    'ambiguous numeric dates prefer month-first unless that is in the future',
    () {
      expect(
        parseReceiptDate('03/04/2026', today: today),
        DateTime(2026, 3, 4),
      );
      expect(
        parseReceiptDate('22/09/2026', today: today),
        DateTime(2026, 9, 22),
      ); // day-first: 22 can't be a month
      expect(
        parseReceiptDate('12/09/2026', today: today),
        DateTime(2026, 9, 12),
      ); // Dec 9 is in the future
      expect(parseReceiptDate('2026-09-01'), DateTime(2026, 9, 1));
      expect(
        parseReceiptDate('Sep 5, 2026', today: today),
        DateTime(2026, 9, 5),
      );
...
''', today: today);
  ...
  group('choosing the line for this item', () {
    const lines = [
      ReceiptLine('SAMSUNG 65" QN90A TV', 1799.99),
      ReceiptLine('HDMI CABLE 2M', 29.99),
    ];
